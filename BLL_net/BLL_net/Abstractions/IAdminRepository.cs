...
public interface IAdminRepository
{
    ...
    Task UpsertSiteTextAsync(string key, string value, CancellationToken ct = default);

    // --- Audit ---
    Task<IReadOnlyList<AdminAuditLogModel>> GetAuditLogsAsync(int limit = 200, CancellationToken ct = default);
    Task AddAuditLogAsync(string tableName, int? recordId, string operation, string? description, CancellationToken ct = default);
}
